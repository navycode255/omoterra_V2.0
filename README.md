...
The explicit development initializer creates the schema; it is not run during API startup. Alternatively apply `backend/migrations/001_initial.sql` once to an empty PostgreSQL database. Do not apply both initialization methods to the same database.
...
- Supplier setup on first stock submission, category-specific stock wizard, review boundary, stock updates/reconfirmation, collection views and payouts.
- Separate sourcing requests and authenticated business setup leads.
...
- Backend-only operator approval, fulfilment, reconciliation, settlement and sourcing conversion endpoints; no operator mobile experience or web dashboard.
