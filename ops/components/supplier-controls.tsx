...
export function SupplierStatusControl({ id, status, canApprove, guide }: { id: string; status: string; canApprove: boolean; guide?: ReactNode }) {
...
export function SupplierVerificationForm({ id, verification, notes }: { id: string; verification: Record<string, boolean>; notes: string }) {
  ...
  return <form action={action} className="verification-form">
    ...
    <div className="verification-checks">{CHECKS.map(([key, label, required]) => <label key={key}><input type="checkbox" name={key} defaultChecked={verification[key] ?? false}/><span>{label}</span>{required && <em className="required-tag" title="Required for approval">Required</em>}</label>)}</div>
