        ...
        <div className="grid-2" style={{ alignItems: 'start' }}>
          ...
          <div className="stack">
            <Card title="Order items">
              {order.items.length === 0 ? <p className="muted small">No items on this order.</p> : <div className="stack">{order.items.map((item) => {
                const supplier = order.suppliers.find((entry) => entry.listing_id === item.listing_id);
                return <div key={item.id} className="card" style={{ boxShadow: 'none' }}><Definition items={[
                  ['Product', category(item.category)], ['Supplier', supplier?.legal_name ?? '—'],
                  ['Reserved', `${quantity(item.quantity)} ${item.unit_type}`], ['Accepted', quantity(item.actual_quantity)],
                  ['Rejected', quantity(item.rejected_quantity)], ['Buyer price', tzs(item.unit_price)],
                  ['Supplier asking price', item.asking_snapshot ? tzs(item.asking_snapshot) : '—'],
                  ['Supplier payout price', item.payout_snapshot ? tzs(item.payout_snapshot) : '—'],
                  ['Original subtotal', tzs(item.subtotal)],
                ]} /></div>;
              })}<Definition items={[["Order total", tzs(order.total_amount)]]} /></div>}
            </Card>
